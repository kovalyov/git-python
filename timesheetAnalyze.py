...
if is_holiday == 'yes':
    ...
    while sum_holiday_day > 0:
        holiday_cell = str(input("Введите номер ячейки с праздником на латинице: \n").upper())
...
for i in timesheets:
    ...
    project_1 = f.read('B8')
    # Ячейка F4 содержит ошибку, в случае если сумма часов за каждый рабочий день не равна 8
    ...
    if len(holiday) > 0:
        for j in holiday:
            holiday_day = f.read(j)  # Проверка праздничных дней
            if holiday_day != 8:
